...
import { ArrowLeft, FileText, Folder, Trash2, Globe, Cpu } from "lucide-react";
import { api, type ScanResult } from "@/lib/api";

type WindowId = "email" | "shield" | "terminal" | "dashboard";
...
export default function Desktop() {
  ...
  const toggleWindow = (id: WindowId) => {
    setWindows((prev) => {
      if (prev[id].closed) return { ...prev, [id]: { closed: false, minimized: false } };
      return { ...prev, [id]: { ...prev[id], minimized: !prev[id].minimized } };
    });
  };

  const openWindow = (id: WindowId) => {
    setWindows((prev) => ({ ...prev, [id]: { closed: false, minimized: false } }));
  };
  ...
  return (
    <div className="w-full h-screen overflow-hidden relative flex flex-col font-sans select-none bg-[#1e1e2e]">
      <div className="absolute inset-0 z-0 bg-[#00173d] overflow-hidden">
        <div className="absolute inset-0 opacity-50 bg-[radial-gradient(ellipse_at_bottom_right,_var(--tw-gradient-stops))] from-[#005bc4] via-[#003087] to-[#00102b]" />
      </div>

      <Link href="/" className="absolute top-3 right-4 z-[60] flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/40 border border-white/10 text-white/70 hover:text-white text-xs transition-colors">
            ...
            <AnimatePresence>
              {!windows.email.minimized && !windows.email.closed && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full h-full">
                  <EmailClient state={state} onMinimize={() => toggleWindow("email")} onClose={() => closeWindow("email")} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          <div className="col-span-12 lg:col-span-5 row-span-2 relative">
            <div className="absolute -inset-4 bg-red-500/5 rounded-3xl blur-xl pointer-events-none transition-opacity duration-300" style={{ opacity: state === SimulationState.SHIELD_INTERVENTION && !windows.shield.minimized && !windows.shield.closed ? 1 : 0 }} />
            <AnimatePresence>
              {!windows.shield.minimized && !windows.shield.closed && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full h-full">
                  <ShieldDashboard state={state} scanResult={scanResult} onMinimize={() => toggleWindow("shield")} onClose={() => closeWindow("shield")} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          <div className="col-span-12 lg:col-span-7 row-span-1">
            <AnimatePresence>
              {!windows.terminal.minimized && !windows.terminal.closed && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full h-full">
                  <Terminal state={state} scanResult={scanResult} onMinimize={() => toggleWindow("terminal")} onClose={() => closeWindow("terminal")} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        ...
        <AnimatePresence>
          {!windows.dashboard.minimized && !windows.dashboard.closed && (
            <div className="w-full h-full pointer-events-auto">
              <AgentDashboard state={state} onMinimize={() => toggleWindow("dashboard")} onClose={() => closeWindow("dashboard")} />
            </div>
          )}
        </AnimatePresence>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: state === SimulationState.SHIELD_INTERVENTION ? 0.15 : 0 }} className="absolute inset-0 bg-red-500 pointer-events-none z-40 transition-opacity duration-300" />
