...
export default function Window({ title, icon, children, className = "", glow = "none", variant = "dark", onMinimize, onClose }: WindowProps) {
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col overflow-hidden shadow-2xl transition-all duration-300 rounded-sm ${isDark ? "bg-[#1e1e1e] border-[#3a3a3a] text-white" : "bg-white border-[#cccccc] text-black"} border ${className} ${glowClass}`}
    >
