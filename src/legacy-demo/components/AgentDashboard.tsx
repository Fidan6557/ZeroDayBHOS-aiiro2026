...
export default function AgentDashboard({ state, onMinimize, onClose }: AgentDashboardProps) {
  const getCoreState = () => {
    switch (state) {
      ...
      case SimulationState.SHIELD_INTERVENTION: return { status: 'Halted', color: 'text-red-500', task: 'Execution blocked by Aegis Shield' };
  ...
  const agents = [
    {
      id: 'REQ-CORE-001',
      name: 'OpenClaude_Core',
      model: 'clau-v4-turbo',
      status: core.status,
      statusColor: core.color,
      task: core.task,
      memory: '4.2 GB / 8.0 GB',
      uptime: '14h 22m',
      icon: <Cpu className={core.color} size={20} />
    },
    {
      id: 'VIS-NODE-042',
      name: 'OpenClaude_Vision',
      model: 'clau-v4-vision',
      status: 'Active',
      statusColor: 'text-blue-400',
      task: 'Processing image batch (folder: /assets/raw)',
      memory: '6.8 GB / 16.0 GB',
      uptime: '02h 14m',
      icon: <Bot className="text-blue-400" size={20} />
    },
    {
      id: 'DEV-ASSIST-11',
      name: 'OpenClaude_CodeGen',
      model: 'clau-v3.5-fast',
      status: 'Idle',
      statusColor: 'text-gray-400',
      task: 'Awaiting IDE trigger...',
      memory: '1.1 GB / 4.0 GB',
      uptime: '4d 10h',
      icon: <Server className="text-gray-400" size={20} />
    }
  ];
  ...
  return (
    <Window
      ...
      onClose={onClose}
      className="h-full shadow-[0_0_100px_rgba(0,0,0,0.8)] border-[#3a3a3a]"
    >
      <div className="flex flex-col h-full bg-[#111111] text-white">
        <div className="flex items-center gap-6 p-6 border-b border-white/5 bg-white/5">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Manage and monitor your local OpenClaude agent instances.</p>
          </div>
