  ...
  return (
    <div className="absolute bottom-0 w-full h-10 bg-[#1e1e1e] border-t border-[#333] flex items-center justify-between z-50 text-white select-none">
      <div className="flex h-full items-center">
        ...
        {/* Search */}
        <div className="h-full flex items-center bg-white/10 w-64 hover:bg-white/20 transition-colors cursor-text group border-b-2 border-transparent hover:border-transparent">
      ...
      <div className="flex h-full items-center">
        <div className="flex items-center h-full">
          <motion.button
            ...
            className={`mr-4 px-3 py-1 rounded text-xs font-bold transition-colors ${
              isSimulating 
        ...
        {/* Show Desktop Button */}
        <div className="h-full w-1 border-l border-white/20 hover:bg-white/20 cursor-pointer"></div>
      </div>
    </div>
  );
}
...
function TaskbarIcon({ icon, active, open, onClick }: { icon: React.ReactNode, active?: boolean, open?: boolean, onClick?: () => void }) {
  return (
    <div onClick={onClick} className={`relative flex items-center justify-center h-full w-12 hover:bg-white/10 transition-colors cursor-pointer ${active ? 'bg-white/5' : ''}`}>
      {icon}
      {open && (
        <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] bg-[#00adef] transition-all duration-200 ${active ? 'w-full' : 'w-2'}`}></div>
      )}
    </div>
  );
}
